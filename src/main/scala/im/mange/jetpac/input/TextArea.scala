...
case class TextArea(field: Field, placeholder: Option[String], default: Option[String]) extends FormInput {
  var value = safeDefault

  private def safeDefault = default getOrElse ""

  def baseElement = SHtml.textarea(value, onSubmit, "id" → id, "style" → styles.render, "class" → classes.render,
